...
| Preferências/última etapa | `etapa_preferencias_concluida`, fluxo ativo inteiro válido antes do backend | navegador | sessão ou usuário | novo submit válido pode repetir | `etapas` por identidade | não é conta criada; falha de autenticação não vira marco porque validação ocorre antes do emissor |
