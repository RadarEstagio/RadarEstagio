...
de prefixos de formação. Um nome desconhecido não herda a área de um trecho do nome, e curso
aceito pela vaga fora do catálogo continua contando como formação de outra área: nas vagas reais
esses nomes são quase sempre curso de outra formação ou nível de ensino, e o sinônimo que falta
entra como alias (16/09/2026). A menção
...
- **A comparação é determinística** (`matching/compatibilidade.py`): `cursos_aceitos` vira
  ...
  superior conta como sempre contou, e item que cita técnico com "ou", "/" ou vírgula vale como
  a lista das alternativas, então "técnico ou superior em X" vale por X);
