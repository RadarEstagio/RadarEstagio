create function public.todos_os_textos_cabem(textos text[], limite integer)
returns boolean
language sql
immutable
strict
set search_path = ''
as $$
  select not exists (select 1 from unnest(textos) texto where char_length(texto) > limite);
$$;

alter table public.perfis
  add constraint perfis_curso_cabe check (char_length(curso) <= 200),
  add constraint perfis_cidade_cabe check (char_length(cidade) <= 120),
  add constraint perfis_habilidades_cabem check (public.todos_os_textos_cabem(habilidades, 100)),
  add constraint perfis_areas_de_interesse_cabem check (cardinality(areas_de_interesse) <= 50);
