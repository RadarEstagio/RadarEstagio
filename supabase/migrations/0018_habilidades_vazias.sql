create function public.habilidades_do_perfil_validas(valor text[])
returns boolean
language sql
immutable
strict
set search_path = ''
as $$
  select cardinality(valor) between 0 and 50
    and array_position(valor, null) is null
    and not exists (
      select 1
      from unnest(valor) habilidade
      where length(btrim(habilidade)) not between 1 and 100
    );
$$;

alter table public.perfis
  drop constraint perfis_habilidades_check;

alter table public.perfis
  add constraint perfis_habilidades_validas
    check (public.habilidades_do_perfil_validas(habilidades));
